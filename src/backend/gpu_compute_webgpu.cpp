...
// REAL ONLY: malloc-backed memory is HOST emulation (no Dawn/wgpu device).
// Compute ops always fail loud via backend wrapper.
...
void GPUComputeWebGPU::gemm(float alpha, const void* A, const void* B, float beta, void* C, int64_t M, int64_t N, int64_t K) {
    // Host reference (NOT Dawn/wgpu execution). Delegates to the tuned reference.
    reference_gemm(alpha, static_cast<const float*>(A), static_cast<const float*>(B),
                   beta, static_cast<float*>(C), M, N, K);
}

// ========================================================================
// Host reference kernels. Tuning: WebGPU workgroups are 256 threads, so the
// host mirror tiles N in 256-wide strips with 64-row groups and K-slices of
// 128 (one dispatch worth of work per tile). i-k-j order. AVX2 (8-wide) on
// host for elementwise work; transcendentals scalar. Beta/alpha-correct,
// null-guarded.
// ========================================================================

void GPUComputeWebGPU::reference_gemm(float alpha, const float* a, const float* b,
    ...
    constexpr int64_t BM = 64, BN = 256, BK = 128;
...
void GPUComputeWebGPU::reference_gemv(float alpha, const float* A, const float* x,
...
void GPUComputeWebGPU::reference_relu(const float* x, float* y, int64_t n) {
    if (!x || !y) return;
    int64_t i = 0;
#if defined(QUANT_AVX2) || defined(__AVX2__)
...
void GPUComputeWebGPU::reference_gelu(const float* x, float* y, int64_t n) {
    if (!x || !y) return;
...
void GPUComputeWebGPU::reference_silu(const float* x, float* y, int64_t n) {
    if (!x || !y) return;
    for (int64_t i = 0; i < n; ++i) {
        const float v = x[i];
...
void GPUComputeWebGPU::reference_add(const float* a, const float* b, float* c, int64_t n) {
    if (!a || !b || !c) return;
    int64_t i = 0;
#if defined(QUANT_AVX2) || defined(__AVX2__)
    for (; i + 8 <= n; i += 8) {
...
void GPUComputeWebGPU::reference_mul(const float* a, const float* b, float* c, int64_t n) {
    if (!a || !b || !c) return;
    int64_t i = 0;
#if defined(QUANT_AVX2) || defined(__AVX2__)
    for (; i + 8 <= n; i += 8) {
...
void GPUComputeWebGPU::reference_scale(float s, const float* x, float* y, int64_t n) {
    if (!x || !y) return;
    int64_t i = 0;
#if defined(QUANT_AVX2) || defined(__AVX2__)
...
void GPUComputeWebGPU::reference_softmax(const float* x, float* y, int64_t rows, int64_t cols) {
...
void GPUComputeWebGPU::reference_rms_norm(const float* x, const float* weight, float* y,
...
void GPUComputeWebGPU::reference_layer_norm(const float* x, const float* gamma, const float* beta,
...
// REAL ONLY: dummy 1GB constants REMOVED (fake capacity); 0 until real
// wgpu adapter queries exist.
