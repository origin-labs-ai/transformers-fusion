...
// REAL ONLY: malloc-backed memory is HOST emulation (no virgl/venus command
// stream exists here). Compute ops always fail loud via backend wrapper.
...
void GPUComputeVirtGPU::gemm(float alpha, const void* A, const void* B, float beta, void* C, int64_t M, int64_t N, int64_t K) {
    // Host reference (NOT virgl/venus execution). Delegates to the tuned reference.
    reference_gemm(alpha, static_cast<const float*>(A), static_cast<const float*>(B),
                   beta, static_cast<float*>(C), M, N, K);
}

// ========================================================================
// Host reference kernels. Tuning: virtio-gpu transfers dominate, so the
// host mirror batches in 64x64 tiles with K-slices of 64 (one transfer
// batch per tile). i-k-j order. AVX2 (8-wide) on host for elementwise
// work; transcendentals scalar. Beta/alpha-correct, null-guarded.
// ========================================================================

void GPUComputeVirtGPU::reference_gemm(float alpha, const float* a, const float* b,
...
void GPUComputeVirtGPU::reference_gemv(float alpha, const float* A, const float* x,
...
void GPUComputeVirtGPU::reference_relu(const float* x, float* y, int64_t n) {
    if (!x || !y) return;
    int64_t i = 0;
#if defined(QUANT_AVX2) || defined(__AVX2__)
...
void GPUComputeVirtGPU::reference_gelu(const float* x, float* y, int64_t n) {
    if (!x || !y) return;
...
void GPUComputeVirtGPU::reference_silu(const float* x, float* y, int64_t n) {
    if (!x || !y) return;
    for (int64_t i = 0; i < n; ++i) {
        const float v = x[i];
...
void GPUComputeVirtGPU::reference_add(const float* a, const float* b, float* c, int64_t n) {
    if (!a || !b || !c) return;
    int64_t i = 0;
#if defined(QUANT_AVX2) || defined(__AVX2__)
    for (; i + 8 <= n; i += 8) {
...
void GPUComputeVirtGPU::reference_mul(const float* a, const float* b, float* c, int64_t n) {
    if (!a || !b || !c) return;
    int64_t i = 0;
#if defined(QUANT_AVX2) || defined(__AVX2__)
    for (; i + 8 <= n; i += 8) {
...
void GPUComputeVirtGPU::reference_scale(float s, const float* x, float* y, int64_t n) {
    if (!x || !y) return;
    int64_t i = 0;
#if defined(QUANT_AVX2) || defined(__AVX2__)
...
void GPUComputeVirtGPU::reference_softmax(const float* x, float* y, int64_t rows, int64_t cols) {
...
void GPUComputeVirtGPU::reference_rms_norm(const float* x, const float* weight, float* y,
...
void GPUComputeVirtGPU::reference_layer_norm(const float* x, const float* gamma, const float* beta,
