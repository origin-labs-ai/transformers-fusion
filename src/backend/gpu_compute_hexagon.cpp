...
void GpuComputeHexagon::launch_gemm(int m, int n, int k, const float* a, const float* b, float* c) {
    // Host path (alpha=1, beta=0). Real HVX dispatch needs hexagon_nn_
    // graph build+execute (not implemented) — see DSPHexagonBackend which
    // fails loud instead of claiming this as DSP work.
    reference_gemm(1.0f, a, b, 0.0f, c, m, n, k);
}

// ========================================================================
// Host reference kernels. Tuning: Hexagon TCM is small and HVX vectors are
// 1024-bit (32 floats); host mirror uses 32-row tiles with a deep K-stream
// (BK=128) so the working set fits tight caches. i-k-j order keeps the C
// row hot. AVX2 (8-wide) used where the op is elementwise; transcendentals
// stay scalar. All routines are beta/alpha-correct and null-guarded.
// ========================================================================

void GpuComputeHexagon::reference_gemm(float alpha, const float* a, const float* b,
    ...
    constexpr int BM = 32, BN = 64, BK = 128;
...
void GpuComputeHexagon::reference_gemv(float alpha, const float* A, const float* x,
...
void GpuComputeHexagon::reference_relu(const float* x, float* y, size_t n) {
    if (!x || !y) return;
    size_t i = 0;
#if defined(QUANT_AVX2) || defined(__AVX2__)
...
void GpuComputeHexagon::reference_gelu(const float* x, float* y, size_t n) {
    if (!x || !y) return;
...
void GpuComputeHexagon::reference_silu(const float* x, float* y, size_t n) {
    if (!x || !y) return;
    for (size_t i = 0; i < n; ++i) {
        const float v = x[i];
...
void GpuComputeHexagon::reference_add(const float* a, const float* b, float* c, size_t n) {
    if (!a || !b || !c) return;
    size_t i = 0;
#if defined(QUANT_AVX2) || defined(__AVX2__)
    for (; i + 8 <= n; i += 8) {
...
void GpuComputeHexagon::reference_mul(const float* a, const float* b, float* c, size_t n) {
    if (!a || !b || !c) return;
    size_t i = 0;
#if defined(QUANT_AVX2) || defined(__AVX2__)
    for (; i + 8 <= n; i += 8) {
...
void GpuComputeHexagon::reference_scale(float s, const float* x, float* y, size_t n) {
    if (!x || !y) return;
    size_t i = 0;
#if defined(QUANT_AVX2) || defined(__AVX2__)
...
void GpuComputeHexagon::reference_softmax(const float* x, float* y, int rows, int cols) {
...
void GpuComputeHexagon::reference_rms_norm(const float* x, const float* gamma, float* y,
...
void GpuComputeHexagon::reference_layer_norm(const float* x, const float* gamma, const float* beta,
