...
Tensor HybridMoeBlock::forward(const Tensor& x, const Tensor& positions,
    ...
    Tensor attn_out;
    Tensor dummy_mask;
    if (kind == HybridAttnKind::KDA) attn_out = kda.forward(h_norm, positions, dummy_mask, cache, layer_idx);
    else attn_out = mla.forward(h_norm, positions, dummy_mask, cache, layer_idx);
...
int64_t HybridMoeBlock::param_count() const {
    ...
    n += (kind == HybridAttnKind::KDA ? kda.param_count() : mla.param_count());
    if (moe) {
