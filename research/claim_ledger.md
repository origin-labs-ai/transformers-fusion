...
| Full-Quick | `ci_full.yml` Ubuntu legs: Quick tests exit code 8 (tests failed) after successful build | Open — local Quick set is 63/63 green with identical flags, so runner-specific (timing/resource/GPU-less-hardware path). Suspects: slow-disk timeouts under `--timeout 120`, `test_gpu_capability` live-dispatch on runner hardware | Open |
