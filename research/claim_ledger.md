...
Still owed (stated, not hidden): GitHub-hosted CI green (pushes done, runs observed via API — see round 5); macOS green (no runner here — `macos.yml` + `ci_full` macOS leg are now accurate); CUDA/Metal/SYCL/HIP runtime (no device on this host); multi-process/NCCL (out of scope, single-host threading tested); end-task quality benches (synthetic MSE only); C-15 draft-present acceptance numbers.

## CI-fix round 5 — 2026-09-13 (GitHub Actions red flags → fixes, API-verified)

Push `314ecf8` triggered the first real GitHub-hosted runs. They exposed
three pre-existing failures (same on `eb3b9e8` — NOT caused by round-4
changes). Status + fixes, all verified via the Actions REST API:

| # | Failure | Root cause | Fix + evidence |
|---|---|---|---|
| W-cfg | `build.yml` Windows job: Configure failed in ~1s (exit 1), two commits in a row | Pinned `-G "Visual Studio 17 2022"` VS-instance discovery fragile across the `windows-latest` (Server 2025) image roll; failure happens before any compile check | **FIXED (commit `c061c9f`, pushed).** Generator VS17 → Ninja + `ilammy/msvc-dev-cmd@v1` + diagnose step (proven in-repo pattern from `release.yml`/`ci_full.yml` Windows legs). API-verified: new run `34758225362` Windows job Configure **GREEN** (12:51:39–48), Build in progress |
| U-build | `build.yml` Ubuntu GCC-13/Clang-18 jobs: Build exit code 2 | Suspect was `bench_full.cpp:574` missing `<dirent.h>` in the POSIX branch (GCC syntax-check confirmed `DIR/opendir/readdir/closedir` undeclared; MSVC masked it via the `_WIN32` branch) — BUT follow-up showed `src/adapters` builds only under `QUANT_BUILD_ADAPTERS=ON` (default OFF) and no CI workflow sets it, so `bench_full` cannot be the CI killer. Real killer still open; local CI-faithful repro (Ninja, `-march=native`, `BENCHMARKS=ON`) builds green + Quick-tests **63/63 green**, so the failure is runner-specific | **PARTIAL.** `dirent.h` fix committed + pushed (`e1bd702`, real portability bug regardless); Ubuntu hunt continues |
| M-build | `macos.yml`: Build failed in 0s (configure green, build instant exit 1, no compile output) | 0-second failure = environment/tool issue, not a compiler error | **INSTRUMENTED (commit `70bcf67`, pushed).** Diagnose-toolchain step + `--verbose` build. Follow-up run: Build went 0s → **50s** before exit 1 — now reaching real Apple Clang compilation. Exact error still hidden (log download needs admin rights); Apple-Clang `immintrin.h` portability-guard agent is working the source side |
| Docker | `ci_full.yml` docker job: Build failed in 1s | Unknown (log access denied); same pre-existing class | Open — Dockerfile itself verified locally accurate (paths + non-root + HEALTHCHECK); rerun owed after source fixes land |
| Full-Quick | `ci_full.yml` Ubuntu legs: Quick tests exit code 8 (tests failed) after successful build | **ROOT-CAUSED + FIXED (commit `87917a7`) + PROVEN ON CI.** CI log (owner-provided) showed **61/62 pass on BOTH GCC-13 and Clang-18, single failure `test_production`**: `tests/test_production.cpp:174` asserted `deploy_android()` returns false, but GitHub-hosted `ubuntu-latest` runners preinstall the Android SDK AND export `ANDROID_HOME`/`ANDROID_SDK_ROOT`, so the impl (correctly) returns true. Classic environment-coupled test: green on bare dev boxes, red on CI. Fix pins the CONTRACT (`result == SDK-detectability`: POSIX = `ANDROID_HOME` set; Win32 = env + `<sdk>/tools/bin/gradlew` exists) instead of the bare-machine value. Verified all paths: Windows no-SDK **35/35**, Windows fake-SDK-no-gradlew **35/35**, Windows fake-SDK-with-gradlew **34/34**, Linux no-SDK **35/35**, Linux `ANDROID_HOME` set (CI sim) **34/34**. **CI-PROVEN 2026-09-13:** `CI Full` run on `7d56066` — Ubuntu GCC-13 Quick tests **SUCCESS** (13:33:49–35:18) and Ubuntu Clang-18 Quick tests **SUCCESS** (13:32:22–33:17). Both legs green with the fix | Open |
...
| F-k3 | K3 deletions (`tools/k3_convert.cpp` + `src/math/math_avx2_tensor.cpp`, owner order) break nothing: fresh Windows MSVC configure + full build + `ctest --test-dir build-k3check -C Release` | **100% tests passed, 0 failed out of 72** (both files were unwired/excluded — zero build impact, as predicted) |

## CI-fix round 6 — 2026-09-13 (`CI Full` remaining red legs → fixes, pushed)

The `CI Full` run on `7d56066` proved the android fix (both Ubuntu Quick legs
**SUCCESS**) but exposed three more red legs in the same workflow. All three
root-caused from step timings + annotations (log download needs admin rights):

| # | Failure | Root cause | Fix (commit `5cadd2a`, pushed) |
|---|---|---|---|
| FW-win | `ci_full` Windows leg (`clang-cl`): Configure exit 1 in ~2s | `CC/CXX: clang-cl` override — same VS-discovery fragility class as the `build.yml` VS17 failure, on the same image. The plain-MSVC Ninja pattern is proven green by `build.yml`'s Windows job | Matrix `cc/cxx: clang-cl` → `msvc` sentinel; Configure step (now `shell: bash` on all OSes) exports NOTHING for `msvc` so CMake picks `cl.exe` from the `msvc-dev-cmd` PATH (empty-string export would break detection). Clang coverage unaffected (clang-18 Linux leg) |
| FW-cov | Coverage step: instant failure (start == complete second) | Step runs `cmake -B build-cov` with Clang-only flags (`-fprofile-instr-generate -fcoverage-mapping`) but sets NO `CC/CXX` — runner default `cc` is gcc → instant reject. (ASAN step survived only because sanitizer flags are GCC-compatible) | `env: CC: clang-18, CXX: clang++-18` on the coverage step |
| FW-mac | `ci_full` macOS leg: Build 0s fail + `macos.yml`: Build ~50s fail | Apple-Clang portability agent's bonus finding: shared Configure forces `-DQUANT_AVX2=ON` and the leg adds `-DCMAKE_CXX_FLAGS="-march=native"` — on Apple Silicon `-mavx2/-mavx/-mbmi2/-mfma` (via `compiler.cmake` QUANT_AVX2 branch) are hard errors | macOS leg drops `-march=native`; Configure computes `AVX2_FLAG` per-OS (`OFF` on macOS, `ON` elsewhere), mirroring `macos.yml`'s ARM-safe flags |
...
**Re-proven 2026-09-14 (full ASan suite, GCC 15.2, same flags as CI):**
`ctest --test-dir build-asan-local` — Quick set **62/62 green** (83.90s) +
heavies **7/7 green** (110.38s), all under
`-fsanitize=address,undefined -fno-sanitize-recover=all`. The CI ASAN step
runs the same Quick set — with the link fix (`a5f58d4`) + RoPE clamp
